...
const renderClustersList = async (ctx: Context, page = 1) => {
  ...
  await pagination<Cluster>({
    ...
    itemsRender: (c) => ({
      ...
      callback_data: `cross_warehouses_page:${c.id}`,
    }),
    callback_data: 'cross_clusters_page',
...
const renderWarehousesList = async (ctx: Context, clusterID: string, page = 1) => {
  ...
  await pagination<Warehouse>({
    ...
    itemsRender: (w) => ({
      ...
      callback_data: `cross_clusters_warehouse_detail:${clusterID}:${w.warehouse_id}`,
    }),
    callback_data: `cross_warehouses_page:${clusterID}`,
...
const renderWarehouseFBOList = async (ctx: Context, clusterID: number, warehouseID: number, page = 1) => {
  ...
  await pagination<WarehouseSearchItem>({
    ...
    itemsRender: (w) => ({
      ...
      callback_data: `cross_warehouse_detail:${clusterID}:${w.warehouse_id}`,
    }),
    callback_data: `cross_clusters_warehouse_detail:${clusterID}:${warehouseID}`,
  ...
  await ctx.editMessageText(text, {
    ...
    reply_markup: {
      inline_keyboard: [
        [
          {
            text: '❤️ Выбрать склад',
            callback_data: `cross_select_warehouse:${clusterID}:${warehouse.warehouse_id}`,
          },
        ],
        [
          {
            text: '🔙 Назад',
            callback_data: `cross_clusters_warehouse_detail:${clusterID}:${warehouse.warehouse_id}`,
          },
        ],
      ],
    },
...
export const setupCrossWarehousesList = (bot: Telegraf) => {
  bot.action('cross_clusters_list', (ctx) => renderClustersList(ctx, 1));
  bot.action(/^cross_clusters_page:(\d+)$/, (ctx) => renderClustersList(ctx, Number(ctx.match[1])));

  bot.action(/^cross_warehouses_page:(\d+)(?::(\d+))?$/, (ctx) => {
    const clusterID = ctx.match[1];
    const page = ctx.match[2] ? parseInt(ctx.match[2]) : 1;
    renderWarehousesList(ctx, clusterID, page);
  });

  bot.action(/^cross_clusters_warehouse_detail:(\d+):(\d+)(?::(\d+))?$/, (ctx) => {
  ...
  bot.action(/^cross_warehouse_detail:(\d+):(\d+)$/, (ctx) => {
    const clusterID = Number(ctx.match[1]);
    const warehouseID = Number(ctx.match[2]);
    renderWarehouseFBODetail(ctx, clusterID, warehouseID);
  });

  bot.action(/^cross_select_warehouse:(\d+):(\d+)$/, (ctx) => {
    const clusterID = Number(ctx.match[1]);
    const warehouseID = Number(ctx.match[2]);
    const storage = singletonMonitoring.getStorage();
    storage.set('drop_off_point_cluster_id', clusterID);
    storage.set('drop_off_point_warehouse_id', warehouseID);
    ctx.answerCbQuery('✅ Склад отгрузки выбран');
